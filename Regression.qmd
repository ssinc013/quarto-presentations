...
Power analysis allows us to detect an effect of a given size with a given degree of confidence. In other words, it helps us determine what sample size will ensure a high probability that we will correctly reject the null hypothesis - that there is no significant relationship between hp, wt and mpg of a car.

If we use sample size recommended by the power analysis, it is the smallest sample size needed to make a good decision for the experiment, given a required significance level, statistical power, and effect size.

For linear models, we can use the following function: `pwrss.f.reg(r2 =, k =, n =, sig.level = , power =)`

-   `r2` is the expected R-squared 
-   `k` is the number of groups and `n` is the total sample size
-   `sig. level` represents a measure of the strength of the evidence, or alpha level
-   `power` of 0.8 indicates that we want at least 80% chance of correctly rejecting the null

Omnibus F test in multiple linear regression is used to test whether R2 is greater than 0 (zero). Assume that we want to predict a continuous variable Y (mpg) using X1 (hp) and X2 (wt) variables. We are expecting that these two variables explain 80% of the variance in the outcome. What is the minimum required sample size?


```{r}

# what sample size do we need from the data to get an R-squared of 0.8 
# and power of 0.8?
pwrss.f.reg(
  # R-squared predicted value
  r2 = 0.8,
  # number of predictors we want to study
  k = 2,
  # sample size we will need
  n = NULL,
  power = 0.8,
  alpha = 0.05
)$n
...
Based on this analysis, our statistical power will be 1. To create a power curve for a fixed sample size, we will vary the R2 values and visualize the change in statistical power. 
...
n = 32 samples, we will fix our effect size there and vary the sample size. 
...
# power curve for fixed effect size
...
Our results show that increasing our sample size increases power. The optimal sample size for our study is approximately 31 samples, which provides an effect size of 0.25 and power = 0.8. 
